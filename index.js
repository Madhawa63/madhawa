...
async function startBot() {
    ...
    const sock = makeWASocket({
        ...
        browser: Browsers.macOS("Chrome"), // Mac Chrome විදිහට WhatsApp එකට පෙන්නීමෙන් බ්ලොක් වීම වළකී
        logger: pino({ level: 'silent' })
    });

    sock.ev.on('creds.update', saveCreds);

    // Pairing Code එක ඉල්ලීම (රෙජිස්ටර් වී නැතිනම් පමණි)
    if (!sock.authState.creds.registered) {
        setTimeout(async () => {
            try {
                ...
                console.log("❌ Pairing Code දෝෂයක්:", err);
            }
        }, 5000);
    }
